...
|---|---|---|---|---|---|---|---|
| `loginUser` | `POST /api/auth/signin` | same | expects `{ accessToken, tokenType, roles }`; stores `accessToken` | returns `{ token, type, id, username, email, roles }`; JWT payload now includes `userId`, roles enum array | public | **Incompatible** | map `token` field, update token parsing/role extraction; remove legacy `accessToken` assumption |
...
| `saveWorkout` | `POST /api/workout` | same | sends full nested exercise object in `workoutSet.exercise`; Date objects serialized | contract requires strict `exercise: { id }`; strict ISO with offset/Z; nonnegative integer measures | auth | **Incompatible (critical)** | transform workout draft to contract shape before submit; validate nonnegative integers |
...
## Explicit incompatible calls to fix first

1. `loginUser` response mapping (`accessToken` -> `token`) and JWT role parsing.
2. `uploadImage` response mapping (`number` -> `{ id }`).
3. `saveWorkout` request shape (`exercise` object -> `{ id }`) and strict schema adherence.
4. `getWorkoutsByDate` path parameter (`ISO datetime` -> `YYYY-MM-DD`) + timezone.
5. `getWorkoutSetsByExerciseId` query (`size/sort` -> `pageSize/sortBy/sortDirection`) + response field updates.
6. Admin user typing (`roles` array object assumption).
