...
| API incompatibility | `workoutService.ts` + workout draft shape | Posts full exercise object in workout sets | Strict request validation failure | Yes | Map local draft models to `{ exercise: { id } }` payload |
...
| Data-loss risk | `WorkoutControls.tsx` | Clears workout on duplicate UUID conflict | User loses unfinished workout | Yes | Keep draft and offer explicit conflict/retry flow |
| Data-loss risk | `App.tsx` | `JSON.parse` without guard for persisted workout | Crash or unusable app state | Yes | Safe parse + versioned schema validation + migration/recovery path |
...
| Maintainability issue | `WorkoutSets.tsx`, `ExerciseDetail.tsx` | Index-based keys | Render instability when order changes | Yes | Stable IDs/uuid keys |
| Maintainability issue | `ExerciseEdit.tsx` | `FieldValues` and casts bypass strong typing | Runtime shape bugs | Yes | Typed form schema inferred from contracts |
...
| Security issue | `useLocalStorage.ts` token storage | JWT stored in localStorage | weak long-term mobile session model | Yes | access token in memory + refresh in SecureStore + backend session revocation |
| Performance issue | multiple components | Imperative fetches without caching/retry policy | repeated network calls, weak offline UX | Yes | TanStack Query with explicit mobile retry/offline rules |
...
| Maintainability issue | `ExerciseEdit.tsx` image flow | replacing image can orphan previous uploaded image | storage leakage risk | Yes | enforce safe upload/replace/cleanup lifecycle |
| Architecture dependency | mobile auth | legacy stateless token model insufficient for long-lived mobile sessions | brittle restoration/revocation behavior | Yes | introduce backend mobile-session endpoints and rotation/revocation semantics before final auth implementation |
